...
#####################################################################################################################   
    def calculate_sector_load(self, sector: Sector):
        ...
        if not getattr(sector, "is_active", True):
            sector.sector_load_attribute = 0.0
            self.db_manager.write_sector_load(sector.sector_id, 0.0)
            return 0.0
...
########################################################################################################################   
    def calculate_cell_load(self, cell: Cell):
        ...
        """
        if not getattr(cell, "IsActive", True):
            cell.cell_load = 0.0
            self.db_manager.write_cell_load(cell.ID, 0.0)
            return 0.0
