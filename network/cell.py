    ...
    def serialize_for_influxdb(self, cell_load):
        try:
            ...
            point = Point("cell_metrics") \
                ...
                .field("has_connected_ues", bool(getattr(self, "HasConnectedUEs", len(self.ConnectedUEs) > 0))) \
...
#########################################################################################
    def add_ue(self, ue):
        if not self.IsActive or not getattr(self.gNodeB, "is_active", True):
            ue_logger.warning(f"Cell {self.ID} is inactive. Cannot add UE with ID {ue.ID}")
            return
    ...
    def update_ue_lists(self):
        ...
        self.HasConnectedUEs = len(self.ConnectedUEs) > 0
