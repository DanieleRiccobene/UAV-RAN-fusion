...
def allocate_to_gnb(gnb, num_ues, sectors, ue_config): 
    if not getattr(gnb, "is_active", True):
        print(f"Warning: Cannot allocate UEs to inactive gNodeB {gnb.ID}.")
        return []
...
def is_sector_available(sector):
    gnb = getattr(sector.cell, "gNodeB", None)
    if not gnb:
        return False
    return (
        getattr(gnb, "is_active", True)
        and getattr(sector.cell, "IsActive", True)
        and getattr(sector, "is_active", True)
        and sector.remaining_capacity > 0
    )
