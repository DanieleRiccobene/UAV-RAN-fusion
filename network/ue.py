...
class UE:
    ...
    def __init__(self, config, **kwargs):
        with UE.ue_lock:
            ...
            self.X = kwargs.get('x_m')
            self.Y = kwargs.get('y_m')
            self.nominal_serving_bs_id = kwargs.get('nominal_serving_bs_id')
            self.prev_bs_id = kwargs.get('prev_bs_id')
            self.chosen_bs_id = kwargs.get('chosen_bs_id')
            self.trace_covered = kwargs.get('covered')
            self.dist_to_serving_bs_m = kwargs.get('dist_to_serving_bs_m')
            self.is_synthetic = bool(kwargs.get('is_synthetic', False))
            self.has_precise_position = bool(kwargs.get('has_precise_position', True))
    ...
    def set_position(self, latitude=None, longitude=None, x_m=None, y_m=None):
    ...
    def update_trace_state(self, sample):
        self.set_position(
            latitude=getattr(sample, "lat", None),
            longitude=getattr(sample, "lon", None),
            x_m=getattr(sample, "x_m", None),
            y_m=getattr(sample, "y_m", None),
        )
        self.prev_bs_id = getattr(sample, "prev_bs_id", None)
        self.chosen_bs_id = getattr(sample, "chosen_bs_id", None)
        self.nominal_serving_bs_id = getattr(sample, "serving_bs_id", None)
        self.trace_covered = getattr(sample, "covered", None)
        self.dist_to_serving_bs_m = getattr(sample, "dist_to_serving_bs_m", None)
        self.is_synthetic = bool(getattr(sample, "synthetic", False))
        self.has_precise_position = bool(getattr(sample, "has_precise_position", True))
    ...
    def serialize_for_influxdb(self):
        try:
            ...
            point = Point("ue_metrics") \
                ...
                .field("latitude", float(self.Latitude) if self.Latitude is not None else 0.0) \
                .field("longitude", float(self.Longitude) if self.Longitude is not None else 0.0) \
                .field("x_m", float(self.X) if self.X is not None else 0.0) \
                .field("y_m", float(self.Y) if self.Y is not None else 0.0) \
                .field("nominal_serving_bs_id", str(self.nominal_serving_bs_id)) \
                .field("prev_bs_id", str(self.prev_bs_id)) \
                .field("chosen_bs_id", str(self.chosen_bs_id)) \
                .field("trace_covered", str(self.trace_covered)) \
                .field("dist_to_serving_bs_m", float(self.dist_to_serving_bs_m) if self.dist_to_serving_bs_m is not None else -1.0) \
